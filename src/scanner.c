 ...
 * Since Synquid is indentation sensitive and uses parse errors to end layouts, this component has many
 * responsibilities.
 *
 ...
 */
void *tree_sitter_synquid_external_scanner_create() {
 ...
 */
bool tree_sitter_synquid_external_scanner_scan(void *payload, TSLexer *lexer, const bool *valid_symbols) {
  Env env = env_new(lexer, valid_symbols, (State*) payload);
  return scan(&env);
}

unsigned tree_sitter_synquid_external_scanner_serialize(void *payload, char *buffer) {
...
void tree_sitter_synquid_external_scanner_deserialize(void *payload, const char *buffer, unsigned length) {
  State *state = (State *) payload;
  Persist p;
  Persist *persist;
  if (length > 0)
...
void tree_sitter_synquid_external_scanner_destroy(void *payload) {
  State *state = (State*) payload;
#ifdef TREE_SITTER_DEBUG
  palette();
